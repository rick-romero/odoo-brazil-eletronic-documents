...
class BaseNfse(models.TransientModel):
    ...
    def send_rps(self):
        self.ensure_one()
        if self.city_code == 'Codigo de São Paulo':  # São Paulo
            
            #Esse aqui sobrescreve para retornar os metodos da NFS-e
            nfse = self._get_nfse_object()
            url = self._url_envio_nfse()

            # _get_client tem que sobrescrever para retornar o valor correto.
            client = self._get_client(url)
            path = os.path.dirname(os.path.dirname(__file__))
            xml_send = render(path, 'envio_rps.xml', nfse=nfse)

            # Esse cara aqui é o elemento que vai servir para assinar o xml
...
<!ATTLIST Lote Id ID #IMPLIED>
            ...
            #TODO Modificar para o estado de São Paulo
            response = client.service.enviarLote(xml_signed)
    ...
    def cancel_nfse(self):
        if self.city_code == 'codigo':  # São Paulo
    ...
    def _url_envio_nfse(self):
        if self.city_code == 'codigo':  # São Paulo
            return 'url de são paulo'        

    # Aqui modifica para setar os valores para a cidade de São Paulo
